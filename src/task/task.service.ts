...
export class TaskService {
  ...
  // CREATE TASK
  async createTask(user: User, body: CreateTaskDto): Promise<Task> {
  ...
  // UPDATE TASK
  async updateTask(userId: number, taskId: number, body: UpdateTaskDto): Promise<Task> {
  ...
  async getNext7DaysTasks(userId: number) {
  ...
  async getAllTasks(dto: GetAllTasksDto) {
    ...
    if (search) {
      query.where('(task.title ILIKE :searchQuery OR task.notes ILIKE :searchQuery)', {
        searchQuery: `%${search}%`
      });
    }
